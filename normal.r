...
## INPUT
d <- read.table(file("stdin"), header=TRUE, fill=TRUE, row.names=NULL)
...
## BUILD KEY (POS + 1)
# pos <- function (x) as.integer(unlist(strsplit(as.character(x[25]),split="[-:]"))[2])
# d$pos <- apply(d,1,pos)

# chr <- function (x) unlist(strsplit(as.character(x[25]),split="[-:]"))[1]
# d$chr <- apply(d,1,chr)

d$key <- paste(d$chr.pos, "_", d$ref.var, sep="")
